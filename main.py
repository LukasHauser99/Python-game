...
#---------------------------------------------------------------------------------------------------------------- 
		...
		self.penize=14000000 #kolik má škola na účtě, integer aby se z toho nedělal float a na obrazovce nebylo x.0
		...
		self.castka=2800   #částka, kterou platí každý student na školném
		...
		self.main=Tk()
		self.main.title("Školní simulátor")
		#fonty		
		...
		self.label_frame.grid(row=2,column=1,padx=90)
		...
		self.studenti_info.grid(column=0,row=0,pady=5,padx=10)
		self.ucitele_info=Label(self.sa_frame,font=self.label_font,text="spokojenost učitelů: ")#píše spokojenost učitelů
		self.ucitele_info.grid(column=0,row=1,pady=5,padx=10)
		self.rodice_info=Label(self.sa_frame,font=self.label_font,text="spokojenost rodičů: ")#píše spokojenost rodičů
		self.rodice_info.grid(column=0,row=2,pady=5,padx=10)
		self.studenti_canvas=Canvas(self.sa_frame,background="white",width=200, height=15) #dělá pole, které ukazuje spokojenost graficky
		...
		self.studenti_canvas.grid(column=1,row=0,pady=5,padx=10)
		...
		self.ucitele_canvas.grid(column=1,row=1,pady=5,padx=10)
		...
		self.rodice_canvas.grid(column=1,row=2,pady=5,padx=10)
		...
		self.plat_button.grid(column=0,row=3,sticky=E+W)
		...
		self.upgrade_button.grid(column=0,row=4,sticky=E+W)
		...
		self.forward_button.grid(column=0,row=0,sticky=W+E)
		...
		self.exit_button.grid(column=0,row=1,sticky=W+E)
...
#----------------------------------------------------------------------------------------------------------------
	def refresh(self): #funkce kontrolující obsah okna
		...
		#úpravy spokojeností
		if self.studenti_spokojenost>100:
			self.studenti_spokojenost=100
		if self.studenti_spokojenost<0:
			self.studenti_spokojenost=0
		...
		if self.rodice_spokojenost>100:
			self.rodice_spokojenost=100
		if self.rodice_spokojenost<0:
			self.rodice_spokojenost=0
		...
		if self.ucitele_spokojenost>100:
			self.ucitele_spokojenost=100
		if self.ucitele_spokojenost<0:
	...
	def popupmsg(self,msg):  #custom msgbox
		self.popup=Toplevel()
		self.popup.grab_set()
		self.popup.title("Pozor") #aby uživatel nemohl do hlavního okna než vyřeší popup
	...
	def upgrades(self): #okno, kde se dá vylepšovat školní vybavení
		...
		self.sport_button.grid(row=0,column=2,padx=10,pady=10,sticky=W+E)#,sticky=E+W)
		self.sport_price=Label(self.upg,font=self.label_font,text="cena vylepšení: "+str((self.sport_level+1)*18000))
		self.sport_price.grid(row=0,column=1,padx=20,pady=10,sticky=W+E)
		...
		self.veda_button.grid(row=1,column=2,padx=10,pady=10,sticky=W+E)
		self.veda_price=Label(self.upg,font=self.label_font,text="cena vylepšení: "+str((self.veda_level+1)*18000))
		self.veda_price.grid(row=1,column=1,padx=10,pady=10)
		...
		self.vedomosti_button.grid(row=2,column=2,padx=10,pady=10,sticky=W+E)
		self.vedomosti_price=Label(self.upg,font=self.label_font,text="cena vylepšení: "+str((self.vedomosti_level+1)*18000))
		self.vedomosti_price.grid(row=2,column=1,padx=10,pady=10)
		...
		self.capacity_button.grid(row=3,column=2,padx=10,pady=10,sticky=W+E)
		self.capacity_price=Label(self.upg,font=self.label_font,text="cena expanze: "+str((self.zaci_lvl)*850000))
		self.capacity_price.grid(row=3,column=1,padx=10,pady=10)
		
		##zpět
		self.b1=Button(self.upg,font=self.label_font,text="zpět",command=self.upg.destroy) #tlačítko zavírající okno
		self.b1.grid(row=4,column=1,padx=20,pady=10,sticky=E+W)
		self.center(self.upg)
	
	def expand(self): #funkce zvyšující kapacitu budovy
		self.penize=self.penize-((self.zaci_lvl)*850000) #počítání ceny na základě stávajícího počtu žáků
		self.money_int.config(text=self.penize)
		self.zaci_lvl+=1 
		...
		self.pocet_ucitelu=self.pocet_zaku/10+10 #aktualizace počtu učitelů
		self.capacity_price.config(text="cena expanze: "+str((self.zaci_lvl)*850000))
		self.refresh()
		
		
	def sport_upgrade(self): #funkce zvyšující úroveň sportovního vybavení
		self.penize=self.penize-((self.sport_level+1)*18000) #počítání ceny a odečítání peněz na základě stávajícího lvlu vybavení
		self.sport_level+=1
		...
		self.sport_price.config(text="cena vylepšení: "+str((self.sport_level+1)*18000))
		return
	
	def veda_upgrade(self): #funkce zvyšující úroveň vědeckého vybavení
		self.penize=self.penize-((self.veda_level+1)*18000) #dynamciká cena odvíjející se od současného lvlu vybavení
		self.veda_level+=1
		...
		self.veda_price.config(text="cena vylepšení: "+str((self.veda_level+1)*18000))
		return
		
	def vedomosti_upgrade(self): #funkce zvyšující úroveň učebních pomůcek
		self.penize=self.penize-((self.vedomosti_level+1)*18000)
		...
		self.vedomosti_price.config(text="cena vylepšení: "+str((self.vedomosti_level+1)*18000))
		return
	...
	def zmenit_skolne(self): #definuje okno, které umožňuje měnit školné 
		self.skol=Toplevel()
		self.skol.grab_set()
		self.skol.title("Změna školného")
	...
	def s_zmen(self): #funkce měnící výši školného
		...
		if (self.s_output>500)and(self.s_output<3500): #kontrola jestli je zadaná hodnota v povoleném rozmezí
	...
	def zmenit_platy(self): #definuje okno umožňující měnit platy učitelů 
		...
		self.plat_wndw.title("Změna platů")
		...
		self.b1=Button(self.plat_wndw,text="Storno",font=self.l_font,command=self.plat_wndw.destroy) #tlačítko zavírající okno
		self.b1.grid(column=0,row=3,padx=10,pady=10,sticky=E+W)
		self.b2=Button(self.plat_wndw,text="Potvrdit",font=self.l_font, command=self.p_zmen) #tlačítko volající funkci, která mění výši platů
		...
		if self.p_output!=0:
			if (self.p_output>self.min_salary): #kontrola, zda je zadaná hodnota nad minimálním platem
				if self.p_output>self.plat:
					self.i=(int(self.p_output)-self.plat)/300
					if self.i==0:
						self.i=1
					...
					self.i=(self.plat-int(self.p_output))/300
					if self.i==0:
						self.i=1
	...
	def competition(self): #meziškolní soutěže
		...
		if max(self.results)==self.playerschool: #když má hráčova škola nejvyšší hodnotu v seznamu, cyklus skončí a spustí se následující kód
			...
			if self.prize!=0:
				...
				self.prize_label.config(text="Nezískal jsi finanční odměnu")
	...
	def event_class1(self,txt,clr,cons):
		...
		self.center(self.random_wndw)
		self.random_wndw.title("Náhodná událost")
		self.random_wndw.configure(background=clr)
		self.event_label=Label(self.random_wndw,bg=clr,font=self.label_font,text=txt)
		self.event_label.grid(row=0,column=0)
		self.consequence_label=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons)
		self.consequence_label.grid(row=1,column=0)
		self.ok=Button(self.random_wndw,text="ok",font=self.label_font,command=self.random_wndw.destroy)
	...
	def event_class2(self,txt,clr,cons1,cons2):
		...
		self.random_wndw.title("Náhodná událost")
		self.random_wndw.configure(background=clr)
		self.event_label_2=Label(self.random_wndw,bg=clr,font=self.label_font,text=txt)
		self.event_label_2.grid(row=0,column=0)
		self.consequence_label1_2=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons1)
		self.consequence_label1_2.grid(row=1,column=0)
		self.consequence_label2_2=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons2)
	...
	def event_class3(self,txt,clr,cons1,cons2,cons3):
		...
		self.random_wndw.title("Náhodná událost")
		self.random_wndw.configure(background=clr)
		self.event_label_3=Label(self.random_wndw,bg=clr,font=self.label_font,text=txt)
		self.event_label_3.grid(row=0,column=0)
		self.consequence_label1_3=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons1)
		self.consequence_label1_3.grid(row=1,column=0)
		self.consequence_label2_3=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons2)
		self.consequence_label2_3.grid(row=2,column=0)
		self.consequence_label3_3=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons3)
	...
	def event_class4(self,txt,clr,cons1,cons2,cons3,cons4):
		...
		self.random_wndw.title("Náhodná událost")
		self.random_wndw.configure(background=clr)
		self.event_label_4=Label(self.random_wndw,bg=clr,font=self.label_font,text=txt)
		self.event_label_4.grid(row=0,column=0)
		self.consequence_label1_4=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons1)
		self.consequence_label1_4.grid(row=1,column=0)
		self.consequence_label2_4=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons2)
		self.consequence_label2_4.grid(row=2,column=0)
		self.consequence_label3_4=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons3)
		self.consequence_label3_4.grid(row=3,column=0)
		self.consequence_label4_4=Label(self.random_wndw,bg=clr,font=self.label_font,text=cons4)
